...
function cgterm_cpus {
    ...
    if [ -z "$1" ]; then
        ...
        if [ "$1" = "all" ]; then
            ...
            if [ -z "$CGTERMS_PERFORMANCE_CPUS" ]; then
            ...
            if [ -z "$CGTERMS_POWERSAVE_CPUS" ]; then
...
function cgterm_memnodes {
    ...
    if [ -z "$1" ]; then
        ...
        if [ "$1" = "all" ]; then
            local nodes=(); readarray -t nodes < <(lscpu --parse=node)
            local last="${nodes[-1]}"
